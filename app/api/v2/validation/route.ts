...
import { baseValidationHandle, exactNormalizedParticipantMatches, normalizeValidationHandle, registerUniqueValidationIdentity, resolveValidationHistoryIdentity } from '@/lib/v2/validationIdentity';
...
export async function POST(request:Request){
 ...
 try{
  if(action==='start'){
   ...
   if(existingId){const existing=await supabaseAdmin.from('validation_participants').select('*').eq('id',existingId).maybeSingle();if(existing.error)throw existing.error;if(existing.data&&baseValidationHandle(String(existing.data.participant_code||''))===normalizeValidationHandle(participantName))participant=existing.data;}
   const normalizedMatches=participant?[]:await exactNormalizedParticipantMatches(participantName);
   if(!participant&&normalizedMatches.length===1)participant=normalizedMatches[0];
   if(!participant){const participantCode=normalizedMatches.length?`${participantName} (${randomUUID().slice(0,4)})`:participantName;const created=await supabaseAdmin.from('validation_participants').insert({participant_code:participantCode,population,known_primary:priorExperience==='YES'?known(body.known_primary):null,known_secondary:priorExperience==='YES'?known(body.known_secondary):null}).select().single();if(created.error)throw created.error;participant=created.data;if(normalizedMatches.length===0)await registerUniqueValidationIdentity(participant.id,participantName);}
   await bindValidationParticipant(participant.id);
   const identity=await resolveValidationHistoryIdentity(participant.id);
   const previous=await supabaseAdmin.from('validation_attempts').select('attempt_number').in('participant_id',identity.historical_participant_ids).order('attempt_number',{ascending:false}).limit(1);
   if(previous.error)throw previous.error;const attemptNumber=(previous.data?.[0]?.attempt_number||0)+1;
   const prior=await supabaseAdmin.from('validation_attempts').select('id,manifest,answers,completed_at,started_at').in('participant_id',identity.historical_participant_ids).order('started_at',{ascending:true});if(prior.error)throw prior.error;
   ...
   const manifest={...assembleValidationAttempt(createSeed(),history,identity),bank_activated_at:environmentAuthority.bank.activated_at};
