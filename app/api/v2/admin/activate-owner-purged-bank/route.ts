...
const EXPECTED_BANK='viago-validation-bank-233-targeted-language-v1.0.0';
const EXPECTED_HASH='98b1f1506971a787e4988c6b297cb39f5c55490c9bc87d1a465bdae5b551d689';
...
export async function POST(request:Request){
 ...
 const {error:authorityError}=await supabaseAdmin.from('validation_runtime_environments').upsert({environment_key:environment,active_validation_bank_id:bank.bank_version,assembler_version:bank.assembler_version,changed_at:activatedAt,changed_by:'OWNER_APPROVED_TARGETED_LANGUAGE_ACTIVATION',change_reason:'Live OWNER targeted retest with deterministic linked history'},{onConflict:'environment_key'});if(authorityError)return respond(`Environment activation failed: ${authorityError.message}`,500);
