...
const EXPECTED_BANK='viago-validation-bank-233-empirical-language-v1.0.0';
const EXPECTED_HASH='95ddea989a454fcfadb475f57e6b35bd192782d129509e278e460b7885a3b790';
...
export async function POST(request:Request){
 ...
 const {error:authorityError}=await supabaseAdmin.from('validation_runtime_environments').upsert({environment_key:environment,active_validation_bank_id:bank.bank_version,assembler_version:bank.assembler_version,changed_at:activatedAt,changed_by:'OWNER_APPROVED_EMPIRICAL_LANGUAGE_ACTIVATION',change_reason:'Live OWNER test of empirical language bank'},{onConflict:'environment_key'});if(authorityError)return respond(`Environment activation failed: ${authorityError.message}`,500);
